...
app.post('/api/admin/login', async(req, res) => {
    try {
        ...
        if (!user) {
            return res.status(401).json({ message: 'Invalid admin credentials or not an admin' });
        }
...
app.post('/api/user/login', async(req, res) => {
    try {
        ...
        if (!user) {
            return res.status(401).json({ message: 'Invalid user credentials or not a regular user' });
        }
...
app.post('/api/register', async(req, res) => {
    try {
        ...
        const userData = {
            ...
            firstName,
            lastName,
            name: `${firstName} ${lastName}`, // Combine for backward compatibility
            phone,
            address,
            city,
            state,
